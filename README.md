...
policy engine (deterministic rules plus LLM-as-judge fallback), full audit logging, and
...
`gemini-3.5-flash-lite` for routing/judging), ChromaDB, SQLAlchemy with PostgreSQL
...
src/
  ...
  router/      LLM-based query classification and multi-domain dispatch
  rag/         chunking (table-aware), embeddings, ChromaDB storage
  policy/      YAML rule engine plus LLM-as-judge fallback and decision gate
